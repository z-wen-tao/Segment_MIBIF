...
data_folder = "D:/desktop/heart_sound/the-circor-digiscope-phonocardiogram-dataset-1.0.3/training_data"
filenames = list()
    ...
    with open(patient_files[i], 'r') as f:
        ...
        for f, j in zip(curr_labels, curr_recording_files):
            ...
            for k_i in range(0, k):
                ...
                mean_en = np.mean(envelopes_split, axis=0)
                std_en = np.std(envelopes_split, axis=0)
                envelopes_split= (envelopes_split- mean_en) / std_en
                ...
                plt.title('t-SNE Visualization of Selected Features (5 Classes)')
